var msgcrypt = (function() {

	_window = null;

	return {
		version : "0.0.1",
		url : "http://vuzzle.googlecode.com/svn/release/usermodules/msgcrypt.js",
		updateUrl : "http://vuzzle.googlecode.com/svn/release/usermodules/versions/msgcrypt.version",
		name : "msgcrypt",
		title : "MsgCrypt",
		description : "Позволяет зашифровывать ваши сообщения.",
		author : "konstantin89",
		icon : null,
		depends : [],
		settings : {
		...
		if(window.location.href.match(/mail.php/)){
		var msgField = document.getElementById('reply_field')||document.getElementById('messageBody');
		var nNav = $('.nNav:first');

		////Кнопка зашифровать..
		
		var a = document.createElement('li');
			a.innerHTML = '<b class="nc"><b class="nc1"><b></b></b><b class="nc2"><b></b></b></b><span class="ncc"><a>Зашифровать</a></span><b class="nc"><b class="nc2"><b></b></b><b class="nc1"><b></b></b></b>';
			a.addEventListener('click',function(){
		
					if($('#cryptKey').val()!==''){
					
						var bf = new Blowfish($('#cryptKey').val());
						msgField.value = (bf.encrypt(msgField.value));
					
					}else{alert('Введите кодовую фразу')}
													},false);
		nNav.append(a);
		
		////Кнопка расшифровать..
		var parsedTxt = document.getElementsByClassName('wrapped')[0].getElementsByTagName('div')[0].textContent.match(/[1|2|3|4|5|6|7|8|9|0|A|B|C|D|E|F]{16,}/g);
					if(parsedTxt){
						var a = document.createElement('a');
							a.innerHTML = '<br>Расшифровать:</br>';
							a.addEventListener('click',function(){
								if($('#cryptKey').val()!==''){
									var bf = new Blowfish($('#cryptKey').val());
									$('.tallRow:eq(3) td:eq(1)').html(bf.decrypt(parsedTxt))
								}else{alert('Введите кодовую фразу')}
																	},false);
							
		$('.tallRow:eq(2) td:eq(1)').append(a);
							};
		////Input ключа
						var a = document.createElement('input');
							a.id = 'cryptKey';
							a.size = '2';
							a.style.backgroundColor = '#DAE2E8';
							a.title = 'Кодовая фраза';
							a.style.marginLeft = '5px';
		nNav.append(a);
		/*//Функция загрузки/сохранения ключа из хранилища=====>
		var savedKeys = vu.data.load('_cryptKeys');
		var json = JSON.parse(savedKeys);
		var toId = $('#to_id').val();

				if(json){
					if(json[toId]){$('#cryptKey').val(json[toId])}
						}
				else{
					vu.data.save('_cryptKeys','{}')
					};

		var a = document.createElement('input');
			a.type = 'radio';
			a.title = 'Сохранить ключ (Можно сохранять подсказку ключа;)(НЕ используйте если у нежелательных людей есть доступ к вашему компьютеру)';
			a.style.marginLeft = '5px';
			a.addEventListener('click',function(){
				json[toId]=$('#cryptKey').val();
				vu.data.save('_cryptKeys',JSON.stringify(json))
													},false);
		nNav.append(a);
								*///<=====
		/* 
...
return unescape(this.unescape(dec));
}
